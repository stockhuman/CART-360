...
// If we are using the hardware SPI interface, these are the pins (for future
// ref)
#define SCLK_PIN 13
#define MOSI_PIN 11
#define CS_PIN 5
...
void testdrawrects(uint16_t color) {
  ...
  for (uint16_t x = 0; x < tft.height() - 1; x += 6) {
    tft.drawRect((tft.width() - 1) / 2 - x / 2, (tft.height() - 1) / 2 - x / 2,
                 x, x, color);
  }
}
...
void testfillrects(uint16_t color1, uint16_t color2) {
  ...
  for (uint16_t x = tft.height() - 1; x > 6; x -= 6) {
    tft.fillRect((tft.width() - 1) / 2 - x / 2, (tft.height() - 1) / 2 - x / 2,
                 x, x, color1);
    tft.drawRect((tft.width() - 1) / 2 - x / 2, (tft.height() - 1) / 2 - x / 2,
                 x, x, color2);
  }
...
void lcdTestPattern(void) {
  static const uint16_t PROGMEM colors[] = {RED,  YELLOW,  GREEN, CYAN,
